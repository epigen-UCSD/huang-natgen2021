...
def slice_file(fl,sx,sy,sz,minx=0,maxx=np.inf,miny=0,maxy=np.inf,minz=0,maxz=np.inf,stridex=1,stridey=1,stridez=1):
    ...
    return data
class dax_im():
    def __init__(self,dax_fl,num_col=None,start_cutoff=0,end_cutoff=0,bead_col=None,color=0):
        #internalize
        self.color = color
    ...
    def start(self,ind_col):
        ...
        #return int(np.ceil((start_cutoff-1)/float(num_col))*num_col+ind_col%num_col)#no +1 correction
        return ind_col
    def get_ims(self,minx=0,maxx=np.inf,miny=0,maxy=np.inf):
        ...
        len_ = np.min(list(map(len,im_blocks)))
...
def mem_map_ims(folders,fov,starts = None):
    ...
    beads_names = list(map(os.path.basename,folders))
...
def get_valid_dax(spots_folder,ifov=0):
    ...
    hybe_tags = np.array(hybe_tags)[np.argsort(list(map(hybe_number,hybe_tags)))]
    fov_tags=[]
    for hybe_tag in hybe_tags:
        fov_tags.extend(list(map(os.path.basename,glob.glob(spots_folder+os.sep+hybe_tag+os.sep+'*.dax'))))

...
class imshow_mark_3d:
    ...
    def press(self,event):
        ...
        if event.key== 'u':
            for i in range(self.index_im,len(self.ims)):
                self.get_seed_points()
                self.index_im = (self.index_im+1)%len(self.ims)
                self.set_image()
        if event.key== 'y':
        ...
        if event.key== '[':
            if hasattr(self,'max_num'):
                self.max_num-=1
                if self.max_num<0:self.max_num=0
        ...
        if event.key.isdigit():
            self.max_num = int(event.key)
            self.update_num_points()
        if event.key== '{':
            if hasattr(self,'dist_th_centers'):
                self.dist_th_centers-=1
                if self.dist_th_centers<0:self.dist_th_centers=0
                self.update_num_points()
        if event.key== '}':
            if hasattr(self,'dist_th_centers'):
                self.dist_th_centers+=1
    ...
    def finish_set_image(self):
        ...
        if self.index_im in self.dic_min_max_c and self.coloron and self.ims_c is not None:
        ...
        if self.index_im in self.dic_min_max and not self.coloron:
    ...
    def fit_seed_points(self):
        ...
        if len(self.pfits>0):
            ...
            if not hasattr(self,'pfits_save'):
                self.pfits_save={}
            if self.index_im in self.pfits_save:
    ...
    def get_seed_points(self):
        ...
        self.th_seed = getattr(paramaters,'th_seed',0)
        hot_pix_th = getattr(paramaters,'hot_pix_th',0)
        
        self.max_num = getattr(paramaters,'max_num',5)
        
        im = self.im_sm
        
        #centers = get_seed_points_base(im,gfilt_size=gfilt_size,filt_size=filt_size,th_seed=th_seed,hot_pix_th=hot_pix_th)
        self.all_centers = get_seed_points_base(im,gfilt_size_min=1,gfilt_size_max=3,filt_size=3,th_seed=0.,max_num=None,
                         use_snr=False,hot_pix_th=hot_pix_th,return_h=True)
        z_min,z_max,x_min,x_max,y_min,y_max = self.get_limits()
        self.all_centers[0] += z_min
        self.all_centers[2] += y_min
        self.all_centers[1] += x_min
        self.update_num_points()
    def prune_centers(self):
        all_centers = self.all_centers
        max_num = self.max_num
        self.dist_th_centers = getattr(self,'dist_th_centers',0)
        dist_th = self.dist_th_centers
        
        all_cents = np.array(all_centers).T
        centers = [all_cents[0]]
        ...
        while True:
            ...
            if counter>len(all_cents)-1:
                break
            if all_cents[counter][-1]<self.th_seed:
                break
    ...
    def update_num_points(self):
        self.centers = self.prune_centers()#self.all_centers[:,:self.max_num]
        centers = self.centers 
        z_min,z_max,x_min,x_max,y_min,y_max = self.get_limits()
        keep = np.array(self.class_ids)!=self.index_im
        not_in_windom = (np.array(self.draw_z)<z_min)|(np.array(self.draw_z)>z_max)|(np.array(self.draw_y)<x_min)|(np.array(self.draw_y)>x_max)|(np.array(self.draw_x)<y_min)|(np.array(self.draw_x)>y_max)
        keep = keep|not_in_windom
        self.class_ids,self.draw_z,self.draw_x,self.draw_y = map(lambda x: list(np.array(x)[keep]),[self.class_ids,self.draw_z,self.draw_x,self.draw_y])
        self.draw_z.extend(centers[0])
        self.draw_x.extend(centers[2])
        self.draw_y.extend(centers[1])
